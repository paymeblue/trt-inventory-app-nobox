...
export const GET = handle(async () => {
  ...
  const rows = await query(
    `SELECT u.id, u.email, u.full_name, u.role, u.job_title, u.phone, u.is_active,
...
export const POST = handle(async (req: Request) => {
  ...
  const row = await queryOne(
    `INSERT INTO users (email, password_hash, full_name, role, phone, job_title)
     VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`,
    [email, hash, fullName, role, b.phone ?? null, b.jobTitle?.trim() || null],
  );
